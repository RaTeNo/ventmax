...
var membtitlt = $(this).find('.memb_title').text();
var membtitxt = $(this).find('.memb_text').text();

$('.member-item-information__title').text(membtitlt);
$('.member-item-information__text').text(membtitxt);
...
$(document).ready(function () {
$(".projects__list input[type='radio']").on("click",function(){
  var check = $(this).attr('id');
  window.location.href = '/projects/'+check;
})
})
$(document).ready( function() {
    $(".contact-form__file").change(function(){
         var filename = $(this).val().replace(/.*\\/, "");
         $(".labelfilename").html(filename);
    });

/*
$('.menu-item-has-children > a').on('click',function(){ 
  if ($(window).width() <= 1199) {
    $(this).parent().parent().find('.sub-menu').slideToggle(300);
    return false;
  }
});
*/
...
$('.searchandfilter .checkbox__input').each(function() {
  if ($(this).prop('checked')) {
    var chktdxt = $(this).siblings('label').html();
    var chktdtd = $(this).attr('id');
    $('.project__tags').prepend('<div class="projects__tag projects__tag_search" id="'+chktdtd+'"><p class="projects__tag_text">#'+chktdxt+'</p><img src="/wp-content/themes/ventmax/img/page-projects/icon.png" alt="" class="projects__tag_img"></div>')
  }
});
$('.project .project__tags').on('click','.projects__tag_search',function(){ 
  var chkdel = $(this).attr('id');
  $('#projects__filter').find('.checkbox__input#'+chkdel).prop( "checked", false );
  var checked = document.querySelectorAll('.searchandfilter .checkbox__input:checked');
  if (checked.length === 0) {
    window.location.replace("/projects/");
  } else {
    $('#projects__filter').find('input[type="submit"]').click();
  }
});
$('.searchandfilter .projects__tag').on('click',' input[type="submit"]',function(e){ 
  var checked = document.querySelectorAll('.searchandfilter .checkbox__input:checked');
  if (checked.length === 0) {
    window.location.replace("/projects/");
  }
...
$('.searchandfilter .checkbox__input').each(function() {
  if ($(this).prop('checked')) {
    $('.projects__tag_clear').show()
  }
});
$('.projects__tag_clear').on('click',function(){ 
  window.location.replace("/projects/");
});